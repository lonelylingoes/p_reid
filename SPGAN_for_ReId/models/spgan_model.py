...
class SPGANModel(BaseModel):
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.criterionMetric = networks.MetricLoss(opt.margin)
            # initialize optimizers
            self.optimizer_G = torch.optim.Adam(itertools.chain(self.netG_A.parameters(), self.netG_B.parameters()),
                                                lr=opt.lr, betas=(opt.beta1, 0.999))
            self.optimizer_D = torch.optim.Adam(itertools.chain(self.netD_A.parameters(), self.netD_B.parameters()),
                                                lr=opt.lr, betas=(opt.beta1, 0.999))
            self.optimizer_M = torch.optim.Adam(self.netMetric.parameters(),
                                                lr=opt.lr, betas=(opt.beta1, 0.999))                                    
            self.optimizers = []
    ...
    def set_input(self, input):
        '''the input should has the matching form'''
    ...
    def optimize_parameters(self):
        ...
        # Metric net
        self.set_requires_grad(self.netMetric, True)
