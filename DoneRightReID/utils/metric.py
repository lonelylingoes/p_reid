...
def mean_ap(
  ...
  m, n = distmat.shape
  # Sort and find correct matches,m x n matric
  indices = np.argsort(distmat, axis=1) 
  # gallery_ids[indices]: m x n matric, every line is sorted by the indices[i] from gallery_ids, 
  #                       each element corresponds to the element of sorted distmat
  # query_ids[:, np.newaxis]: expend query_ids to a matric of shape m x 1
  # matches: m x n matric, indicates for every query, the matched gallery is existed or not
  matches = (gallery_ids[indices] == query_ids[:, np.newaxis])
  ...
  is_valid_query = np.zeros(m)
  for i in range(m):
    # for each query, filter out [the same id and same camera] of each gallery
    # this judgment may could be contrled as a optional param 'filet_same_camera'
    valid = ((gallery_ids[indices[i]] != query_ids[i]) |
             (gallery_cams[indices[i]] != query_cams[i]))
    # y_true: get the valid elements of matches[i]
    y_true = matches[i, valid]
    # distmat[i][indices[i]] is the sorted distance vector for query i,
    # distmat[i][indices[i]][valid] get the valid elements, 
    #             multply -1, the shorter distance gets the higher score
