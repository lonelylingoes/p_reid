...
def read_data_set(filename_queue):
    ...
    #图片的尺寸，这取决于之前数据预处理的
    label_bytes = 2 #表明是negtive还是positive
    result.height = IMAGE_HEIGHT 
    result.width = IMAGE_WITH
    result.depth = 3
    image_bytes = result.height * result.width * result.depth
    record_bytes = (label_bytes + image_bytes) * 2#一张图像

    #定义一个reader,每次从文件中读取固定*字节数*
    ...
    result.label = tf.cast(tf.strided_slice(record_bytes, [0], [1]), tf.int32)#第二个参数表示截取的开始位置，第三个参数表示截取结束位置
    depth_major = tf.cast(tf.strided_slice(record_bytes, [1], [1 + image_bytes]), tf.uint8)
    depth_major = tf.reshape(depth_major , [result.depth, result.height, result.width])  
