#-*- coding:utf-8 -*-
#===================================
#cuhk03 data set prepare program
#===================================
...
def save_images(mat_file, save_dir, new_im_name_tmpl):
    ...
    return labeled_im_dir, detected_im_dir
...
def transform(zip_file, train_test_partition_file, save_dir=None):
    ...
    labeled_im_dir, detected_im_dir = save_images(mat_file, save_dir, new_im_name_tmpl)
    ...
    for im_type in ['detected', 'labeled']:
        im_dir = labeled_im_dir if im_type == 'labeled' else detected_im_dir

        trainval_im_names = train_test_partition[im_type]['train_im_names']

        trainval_ids = list(set([parse_original_new_im_name(n, 'id')
        ...
        train_val_partition = partition_train_val_set(trainval_im_names, parse_original_new_im_name, val_ids_num=100)
        train_im_names = train_val_partition['train_im_names']
        train_ids = list(set([parse_original_new_im_name(n, 'id')
        ...
        test_im_names = [osp.join(im_dir, name) for name in test_im_names]
        val_im_names = [osp.join(im_dir, name) for name in val_im_names]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--zip_file',
        type=str,
        default='/data/DataSet/cuhk03/cuhk03_release.zip')
    parser.add_argument(
        '--save_dir',
        type=str,
        default='/data/DataSet/cuhk03')
    parser.add_argument(
        '--train_test_partition_file',
        type=str,
        default='/data/DataSet/cuhk03/re_ranking_train_test_split.pkl')
