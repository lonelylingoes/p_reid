...
#         if current_epoch < 60:
#             lr = learning_rate
#         elif current_epoch >= 60 and current_epoch< 120:
#             lr = 1e-4
#         elif current_epoch >= 120:
#             lr = 1e-5
# =============================================================================
        f = lambda:tf.cond(tf.logical_and(current_epoch >= 60 , current_epoch< 120),
                     lambda:tf.constant(1e-4),
                     lambda:tf.constant(1e-5))
        lr = tf.cond(current_epoch < 60, lambda:tf.constant(learning_rate), f)
        return lr
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        resnetX_feature_map, resnetX_global_feaures = model.resnetX_inference(images, tf.estimator.ModeKeys.TRAIN)
        # Calculate loss.
