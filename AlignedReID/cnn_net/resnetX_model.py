...
"""
Resnet-Xception replaces the 3x3 filter kernel through the Xception cell,
which contains 3x3 channel-wise convolution layer and one 1x1 spatial convolution layer.
see in "Xception deep learning with depthwise separable convolutions"
"""
...
def building_block(inputs, filters, is_training, projection_shortcut, strides,
  ...
  inputs = depthwise_sepa_conv2d_fixed_padding(
...
def imagenet_resnetX_v2_generator(block_fn, layers, num_classes,
                                 data_format=None):
...
def get_model_param(resnet_size):
  '''
  根据模型尺寸获取模型参数
  input:
    resnet_size:网络尺寸
  ...
  return model_params[resnet_size]



def imagenet_resnetX_v2(resnet_size, data_format=None):
  """Returns the ResNet model for a given size and number of output classes."""
  params = get_model_param(resnet_size)
  return imagenet_resnetX_v2_generator(params['block'], params['layers'], data_format)
